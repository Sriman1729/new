...
const getFarmingAdvice = (weather) => {
  if (!weather) return "Conditions are normal. Continue with routine farm activities.";
  ...
  if (temp >= 20 && temp <= 30 && humidity >= 50 && humidity <= 70)
    return "Good for wheat sowing. Ensure timely planting.";
  if (temp >= 25 && temp <= 35 && condition.includes("clear"))
    return "Good for paddy transplanting. Ensure water supply.";
  if (condition.includes("rain"))
    return "Rain detected. Postpone harvesting and prepare drainage.";
  if (temp > 35) return "High heat. Provide irrigation and shade.";
  if (humidity > 80) return "High humidity. Watch for paddy blast and rust.";
  if (temp < 15) return "Cold conditions. Protect sensitive crops.";
  return "Conditions are normal. Continue with routine farm activities.";
};
...
const generateFarmingAlerts = (current, forecast) => {
  ...
  if (temp > 40)
    newAlerts.push({
      ...
      message:
        "Extreme heat may damage crops. Provide emergency irrigation.",
    });
  else if (temp < 5)
    newAlerts.push({
      type: "warning",
      icon: <Snowflake />,
      title: "Frost Alert",
      message: "Protect potato, sugarcane and citrus crops immediately.",
    });
  if (humidity > 85)
    newAlerts.push({
      type: "warning",
      ...
      message: "High humidity increases disease risks in crops.",
    });
  if (windSpeed > 15)
    newAlerts.push({
      type: "warning",
  ...
  if (rainForecast)
    newAlerts.push({
      ...
      icon: <CloudRain />,
      title: "Rain Expected",
      message: "Rain in next 24h. Good for sowing, postpone harvest.",
    });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6 animate-fade-in">
          ...
          <aside className="flex flex-col gap-6">
            {farmingAlerts.length > 0 && (
              <AlertsPanel alerts={farmingAlerts} />
            )}
          </aside>
        </div>
      )}
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
        {dailyForecast.map((day, index) => (
          <div
            key={index}
            className="text-center p-3 rounded-lg bg-gray-100 dark:bg-gray-700 flex flex-col items-center"
          >
